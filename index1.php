...
if (!isset($_SESSION['hosiname']) && !isset($_SESSION['hosiname1']) ) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['hosiname'];
  $filter2 = $_SESSION['hosiname1'];
...
<!DOCTYPE html>
  ...
  <title>Anisa - Hospital Representative Homepage</title>
...
<?php
$filter1 = $row1['Address'];
$sql = "SELECT `Booking_ID`, `User_ID`, `Driver_ID`, `Hospital_ID`, `Ambulance_ID`, `Details`, `Date_Time`, `Created_At`, `Status` FROM `bookings` WHERE `Hospital_ID` = '$filter2' OR `Status` = 'Pending'";
...
<?php
  ...
  ?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to pay for this booking?')?window.location.href='insertion.inc.php?action=payB&id=<?php echo($row["Booking_ID"]); ?>':true;" title='Confirm Payment for Booking'>Confirm Payment</button></td>
<?php
