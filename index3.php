...
<!DOCTYPE html>
  ...
  <title>Anisa - User Homepage</title>
...
<br>
            <div class="col-md-12">
              <div>
                <p><a onclick="printData1();" class="btn btn-primary">Print</a></p>
                <br>
              </div>
            </div>
    </div> 
...
$sql = "SELECT `Booking_ID`, `User_ID`, `Driver_ID`, `Hospital_ID`, `Ambulance_ID`, `Details`, `Date_Time`, `Created_At`, `Status` FROM `bookings` WHERE `User_ID` = '$filter2'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<?php
}else if ((isset($_SESSION['username']) && ($row['Status'] == "Completed"))) {
  //User Can Cancel Booking
  ?>
<td></td>
<?php
}else if ((isset($_SESSION['username']) && ($row['Status'] != "Paid For"))) {
  //User Can Cancel Booking
  ?>
...
?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to delete this booking?')?window.location.href='insertion.inc.php?action=deleteB&id=<?php echo($row["Booking_ID"]); ?>':true;" title='Delete Booking'>Delete</button></td>
</tr>
<?php
}
} else { echo "No results"; }
...
<br>
            <div class="col-md-12">
              <div>
                <p><a onclick="printData2();" class="btn btn-primary">Print</a></p>
                <br>
              </div>
            </div>
    </div>  
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <br>
                <input class="form-control" min="<?php echo date('Y-m-d'); ?>" type="date" required name="date">
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
            ...
            <div class="col-md-12">
              <div class="form-group">
                <select class="form-control" required name="aid">
                  <option value="" disabled selected>Kindly Select An Ambulance ID</option>
                                     <?php
                                      $sql = "SELECT `users`.*, `ambulance`.* FROM `ambulance` JOIN `users` ON `ambulance`.`User_ID` = `users`.`User_ID`";
                                      $all_categories = mysqli_query($conn,$sql);
                                      while ($category = mysqli_fetch_array(
                                              $all_categories,MYSQLI_ASSOC)):;
                                  ?>
                                  <option value="<?php echo $category["Ambulance_ID"];?>,<?php echo $category["Address"];?>"><?php echo $category["Ambulance_ID"];?> at <?php echo $category["Address"];?></option>
                                  <?php
                                      endwhile;
                                  ?>
                </select>
            ...
            <div class="col-md-12">
              <div class="form-group">
                <textarea name="det" required class="form-control" id="" cols="30" rows="7" placeholder="Description for Booking..."></textarea>
              </div>
            </div>
            <div class="col-md-12">
              <div class="form-group">
                <input value="Submit" name="bookA" class="btn btn-primary" type="submit">
              </div>
            </div>
          </div>
        </form>
