...
if (!isset($_SESSION['driname']) && !isset($_SESSION['driname1']) ) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['driname'];
  $filter2 = $_SESSION['driname1'];
  $query=mysqli_query($conn,"SELECT * FROM `users` WHERE `User_ID`='$filter'")or die(mysqli_error());
  $row1=mysqli_fetch_array($query);
  $query=mysqli_query($conn,"SELECT * FROM `ambulance` WHERE `User_ID`='$filter'")or die(mysqli_error());
  $row2=mysqli_fetch_array($query);  
  $filter1 = $row2['Ambulance_ID'];
}
?>
...
<!DOCTYPE html>
  ...
  <title>Anisa - Ambulance Driver Homepage</title>
...
$sql = "SELECT `Ambulance_ID`, `User_ID`, `Price`, `Reg_No`, `Image` FROM `ambulance` WHERE `User_ID` = '$filter'";
...
$sql = "SELECT `Booking_ID`, `User_ID`, `Driver_ID`, `Hospital_ID`, `Ambulance_ID`, `Details`, `Date_Time`, `Created_At`, `Status` FROM `bookings` WHERE `Driver_ID` = '$filter2' OR `Ambulance_ID` = '$filter1'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
